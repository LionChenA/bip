...
export function SVGTimeline({ items, activeSlug }: SVGTimelineProps) {
  const processedData = useMemo(() => {
    ...
    sortedYears.forEach(year => {
      ...
      sortedMonths.forEach(month => {
        ...
        const monthSpacing = Math.pow(noteCount, k) * spacing;
  ...
  return (
    <div className="fixed left-0 top-0 h-full w-48 hidden lg:flex flex-col py-20 pl-8 select-none z-10 overflow-y-auto scrollbar-hide">
      <svg width="150" height={processedData.totalHeight} className="overflow-visible" aria-label="Garden Timeline">
        <title>Garden Timeline</title>
        <line x1="10" y1="0" x2="10" y2={processedData.totalHeight} className="stroke-foreground/20" strokeWidth="6" />
        <line x1="30" y1="0" x2="30" y2={processedData.totalHeight} className="stroke-border" strokeWidth="4" />
              ...
              {el.type === 'year' ? (
                ...
                <g className="group cursor-pointer">
                  <line 
                    ...
                    x2="30" 
                    y2={el.y} 
                    className={isActive ? "stroke-primary" : "stroke-border group-hover:stroke-primary/50"} 
                    strokeWidth="2" 
                  />
                  <text 
                    x="40" 
